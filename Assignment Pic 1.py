...
def count_objects(image_path, min_area=700):
...
"""
## STATEMENT 5:The average size of the objects in the scene is: X
import cv2

def find_average_object_size(image_path, min_area=500, aspect_ratio_range=(0.3, 2.0)):
    # Load the image and convert it to grayscale
    ...
    # Filter out contours with area smaller than min_area and with aspect ratios outside the desired range
    filtered_contours = []
    for c in contours:
...
def find_most_common_color(image_path, output_image_path, morph_iterations=2):
    ...
    cv2.imshow("Most Common Color", highlighted_image)
    cv2.waitKey(0)
    cv2.destroyAllWindows()
...
def find_object_with_highest_aspect_ratio(image_path, morph_iterations=2, min_area=50, aspect_ratio_range=(1, 10)):
    ...
    # Draw the bounding rectangle around the object with the highest aspect ratio
    if max_aspect_ratio_contour is not None:
        x, y, w, h = cv2.boundingRect(max_aspect_ratio_contour)
        cv2.rectangle(image, (x, y), (x + w, y + h), (0, 255, 0), 2)
        cv2.imwrite("highest_aspect_ratio_object.jpg", image)

        # Display the output image
        cv2.imshow("Highest Aspect Ratio Object", image)
        cv2.waitKey(0)
        cv2.destroyAllWindows()
...
def display_result(image_path, contour):
    image = cv2.imread(image_path)
    x, y, w, h = cv2.boundingRect(contour)
    cv2.rectangle(image, (x, y), (x + w, y + h), (0, 255, 0), 2)
    cv2.imshow("Lowest Aspect Ratio Object", image)
    cv2.imwrite("lowest_aspect_ratio_object.jpg", image)
    cv2.waitKey(0)
    cv2.destroyAllWindows()
...
display_result(image_path, lowest_aspect_ratio_contour)
"""
"""
...
def find_object_closest_to_center(image_path, morph_iterations=2, blur_size=5, canny_low_threshold=50, canny_high_threshold=150):
    # Load the image
    image = cv2.imread(image_path)
    ...
    # Apply morphological closing operations to fill in small gaps
    kernel = cv2.getStructuringElement(cv2.MORPH_RECT, kernel_size)
    closed_image = cv2.morphologyEx(edges_image, cv2.MORPH_CLOSE, kernel, iterations=morph_iterations)
...
def display_result(image_path, contour):
    ...
    cv2.imshow("Object Closest to Center", image)
    cv2.imwrite("object_closest_to_center.jpg", image)
    cv2.waitKey(0)
    cv2.destroyAllWindows()
...
display_result(image_path, closest_contour)
"""
"""
...
def find_object_farthest_from_center(image_path, min_area_ratio=0.001, min_solidity=0.5):
    # Load the image
    image = cv2.imread(image_path)
    ...
    for contour in contours:
        ...
        if hull_area > 0:  # Add this condition to prevent division by zero
    ...
    return farthest_contour, max_distance


def display_result(image_path, farthest_contour):
    image = cv2.imread(image_path)
    cv2.drawContours(image, [farthest_contour], -1, (0, 255, 0), 2)
    cv2.imshow("Object Farthest from Center", image)
    cv2.imwrite("object_farthest_from_center.jpg", image)
    cv2.waitKey(0)
    cv2.destroyAllWindows()
...
print("Distance to the center from farthest object:", farthest_distance)
display_result(image_path, farthest_contour)
"""
"""
...
def iou(rect1, rect2):
    ...
    x1_intersection = max(x1, x2)
    y1_intersection = max(y1, y2)
...
def find_occluded_objects(image_path, iou_threshold=0.2):
    image = cv2.imread(image_path)
    gray_image = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
    ...
    for i, rect1 in enumerate(bounding_rectangles):
        for j, rect2 in enumerate(bounding_rectangles):
            if i != j and (i, j) not in occluded_pairs and (j, i) not in occluded_pairs:
...
def find_average_distance_between_objects(image_path):
    ...
    for contour in contours:
        area = cv2.contourArea(contour)
        if area > 1000:  # Filter out small contours and noise
    ...
    for i in range(num_centroids):
        for j in range(i + 1, num_centroids):
            ...
            cv2.line(image, centroids[i], centroids[j], (0, 255, 0), 1)

    # Draw centroids
    for centroid in centroids:
        cv2.circle(image, centroid, 3, (255, 0, 0), -1)
...
image_path = 'NIVETHANRAMESH_TP062192_PIC2.jpg'
average_distance, image_with_objects = find_average_distance_between_objects(image_path)
...
def find_total_perimeter(image_path):
    ...
    return total_perimeter, image

image_path = 'NIVETHANRAMESH_TP062192_PIC2.jpg'
total_perimeter, image_with_contours = find_total_perimeter(image_path)
...
def find_average_perimeter(image_path):
    ...
    kernel = np.ones((3, 3), np.uint8)
    morphed_image = cv2.morphologyEx(thresholded_image, cv2.MORPH_CLOSE, kernel, iterations=3)
    morphed_image = cv2.morphologyEx(morphed_image, cv2.MORPH_OPEN, kernel, iterations=3)
    ...
    if num_valid_contours == 0:
        ...
        return total_perimeter / num_valid_contours, image

image_path = 'NIVETHANRAMESH_TP062192_PIC2.jpg'
average_perimeter, image_with_contours = find_average_perimeter(image_path)
...
def find_object_with_most_neighbors(image_path, distance_threshold_ratio=0.15):
    morphed_image = preprocess_image(image_path)
    contours, _ = cv2.findContours(morphed_image, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    ...
    for i, contour in enumerate(filtered_contours):
        color = (0, 255, 0) if i == max_neighbor_index else (255, 0, 0)
        cv2.drawContours(output_image, [contour], -1, color, 2)
        cv2.putText(output_image, f'ID: {i}', tuple(centroids[i]), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 255), 2)
    ...
    return max_neighbor_index, max_neighbors
...
print("Centroid of the object with the most neighbors:", object_with_most_neighbors)
"""
"""
...
def find_most_common_relationship(image_path, min_area):
    image = cv2.imread(image_path)
    gray_image = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
    ...
    valid_centroids = [centroid for i, centroid in enumerate(centroids) if stats[i, cv2.CC_STAT_AREA] > min_area]

    relationship_count = {'right': 0, 'left': 0, 'below': 0, 'above': 0}
    for i, centroid in enumerate(valid_centroids):
        for other_centroid in valid_centroids:
...
def draw_relationships(image_path, centroids, most_common_relationship):
    image = cv2.imread(image_path)
    output_image = image.copy()

    for centroid in centroids:
        cv2.circle(output_image, (int(centroid[0]), int(centroid[1])), 5, (0, 255, 0), -1)

        if most_common_relationship == 'right':
            cv2.arrowedLine(output_image, (int(centroid[0]), int(centroid[1])), (int(centroid[0]) + 30, int(centroid[1])), (0, 0, 255), 2)
        elif most_common_relationship == 'left':
            cv2.arrowedLine(output_image, (int(centroid[0]), int(centroid[1])), (int(centroid[0]) - 30, int(centroid[1])), (0, 0, 255), 2)
        elif most_common_relationship == 'below':
            cv2.arrowedLine(output_image, (int(centroid[0]), int(centroid[1])), (int(centroid[0]), int(centroid[1]) + 30), (0, 0, 255), 2)
        else:  # most_common_relationship == 'above'
            cv2.arrowedLine(output_image, (int(centroid[0]), int(centroid[1])), (int(centroid[0]), int(centroid[1]) - 30), (0, 0, 255), 2)

    cv2.imshow('Most Common Relationship', output_image)
    cv2.waitKey(0)
    cv2.destroyAllWindows()

image_path = 'NIVETHANRAMESH_TP062192_PIC2.jpg'
min_area = 100
most_common_relationship, count = find_most_common_relationship(image_path, min_area)
_, _, _, centroids = cv2.connectedComponentsWithStats(cv2.morphologyEx(cv2.adaptiveThreshold(cv2.GaussianBlur(cv2.cvtColor(cv2.imread(image_path), cv2.COLOR_BGR2GRAY), (5, 5), 0), 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY_INV, 11, 2), cv2.MORPH_OPEN, np.ones((3, 3), np.uint8)))
valid_centroids = [centroid for i, centroid in enumerate(centroids) if cv2.connectedComponentsWithStats(cv2.morphologyEx(cv2.adaptiveThreshold(cv2.GaussianBlur(cv2.cvtColor(cv2.imread(image_path), cv2.COLOR_BGR2GRAY), (5, 5), 0), 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY_INV, 11, 2), cv2.MORPH_OPEN, np.ones((3, 3), np.uint8)))[2][i, cv2.CC_STAT_AREA] > min_area]

draw_relationships(image_path, valid_centroids, most_common_relationship)

print("The most common spatial relationship is", most_common_relationship, "with", count, "occurrences.")
"""
"""
## STATEMENT 23 :The largest group/cluster of objects in the scene has X objects
import cv2
import numpy as np
...
def simple_clustering(centroids, distance_threshold, min_samples, max_distance_ratio):
    clusters = []
    ...
    for i, centroid in enumerate(centroids):
        ...
        for j, other_centroid in enumerate(centroids):
            distance = euclidean_distance(centroid, other_centroid)
            if i != j and distance < distance_threshold:
                if max_distance_ratio is not None:
                    direct_distance = euclidean_distance(centroids[0], other_centroid)
                    centroid_distance = euclidean_distance(centroids[0], centroid)
                    if centroid_distance == 0 or direct_distance / centroid_distance > max_distance_ratio:
                        continue
                neighbors.append(j)

        if len(neighbors) >= min_samples - 1:
            cluster = [centroid] + [centroids[j] for j in neighbors]
...
def find_largest_cluster(image_path, distance_threshold, min_samples, max_distance_ratio):
    image = cv2.imread(image_path)
    gray_image = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
    ...
    clusters = simple_clustering(centroids, distance_threshold, min_samples, max_distance_ratio)
...
num_objects_in_largest_cluster = find_largest_cluster(image_path, distance_threshold, min_samples, max_distance_ratio)

print("The largest group/cluster of objects in the scene has", num_objects_in_largest_cluster, "objects.")
"""
"""
## STATEMENT 24 :The smallest group/cluster of objects in the scene has X objects
import cv2
import numpy as np
...
"""
"""
## STATEMENT 25 :The average number of objects per group/cluster in the scene is: X
import cv2
import numpy as np
...
"""
"""
## STATEMENT 26 :The number of objects with a specific orientation (e.g., horizontal, vertical, or diagonal) is: X
import cv2
import numpy as np
...
"""
"""
## STATEMENT 27 :The average orientation of the objects in the scene is: X
import cv2
import numpy as np
...
"""
"""
## STATEMENT 28 :The object with the highest color intensity is id: X
import cv2
import numpy as np
...
"""
"""
## STATEMENT 29 :The object with the lowest color intensity is id: X
import cv2
import numpy as np
...
"""
"""
## STATEMENT 30 :The average color intensity of the objects in the scene is: X
import cv2
import numpy as np
...
"""
"""
## STATEMENT 31 :The number of objects with a specific level of brightness (e.g., low brightness, high brightness) is: X
import cv2
import numpy as np
...
"""
"""
## STATEMENT 32 :The average brightness of the objects in the scene is: X
import cv2
import numpy as np
...
"""
"""
## STATEMENT 33 :The number of objects with a specific level of contrast with their surroundings is: X
import cv2
import numpy as np
...
"""
"""
## STATEMENT 34 :The object with the highest contrast with its surroundings is id: X
import cv2
import numpy as np
...
def object_with_highest_contrast(image_path):
    ...
    for label in range(1, num_labels):
        ...
        surrounding_mask = cv2.dilate(mask, cv2.getStructuringElement(cv2.MORPH_RECT, (7, 7)), iterations=1)
        surrounding_mask = cv2.subtract(surrounding_mask, mask)

        object_pixels = cv2.bitwise_and(gray_image, gray_image, mask=mask)
        surrounding_pixels = cv2.bitwise_and(gray_image, gray_image, mask=surrounding_mask)

        diff_image = cv2.absdiff(object_pixels, surrounding_pixels)
        contrast = np.sum(diff_image)
    ...
    x, y, w, h, _ = stats[max_contrast_label]
...
"""
"""
## STATEMENT 35 :The number of objects with a specific hue range (e.g., between 0 and 60) is: X
import cv2
import numpy as np
...
"""
## STATEMENT 36 :The number of objects with a specific aspect ratio range (e.g., between 1 and 2) is: X
import cv2
import numpy as np
...
"""
"""
## STATEMENT 37 :The object with the highest number of corners/vertices is id: X
import cv2
import numpy as np
...
def find_object_with_most_corners(image_path):
    ...
    for contour in contours:
        epsilon = 0.03 * cv2.arcLength(contour, True) # Increase epsilon value to reduce overestimation
    ...
    x, y, w, h = cv2.boundingRect(max_corners_object)
    cv2.rectangle(image, (x, y), (x + w, y + h), (0, 255, 0), 2)
...
"""
"""
## STATEMENT 38 :The number of objects with a specific saturation range (e.g., between 0.3 and 0.6) is: X
import cv2
import numpy as np
...
"""
"""
## STATEMENT 39 :The average number of corners/vertices per object in the scene is: X
import cv2
import numpy as np
...
"""
"""
## Statement 40: Implement an unsupervised learning approach to cluster objects based on their visual properties
...
def extract_object_features(image_path, min_area=100):
    ...
    gray_image = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
    blurred_image = cv2.GaussianBlur(gray_image, (5, 5), 0)
    edged_image = cv2.Canny(blurred_image, 50, 250)
...
"""
##Statement 41: Apply a rule-based approach to identify common object patterns in the scene, such as objects that are arranged in a grid or objects that are clustered together.
import cv2
...
def detect_shapes(image_path):
    ...
    shape_counts = {"triangle": 0, "square": 0, "rectangle": 0, "circle": 0, "unknown": 0}
    ...
    cv2.imwrite("output_shapes.jpg", image)
    cv2.waitKey(0)
